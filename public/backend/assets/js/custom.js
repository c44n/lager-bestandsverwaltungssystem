document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchProducts(query, warehouse_id) {
        fetch(productSearchUrl + "?query=" + query + "&warehouse_id=" + warehouse_id).then(response => response.json()).then(data => {
            product_list.innerHTML = "";
            if (data.length > 0) {
                ...
                product_list.innerHTML = '<p class="text-muted">Kein Produkt gefunden</p>'
            }
        })
    }
